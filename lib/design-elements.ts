...
export const ELEMENT_CATEGORIES: { id: string; label: string }[] = [
  ...
  { id: "outils", label: "Outils & bricolage" },
  { id: "science", label: "Science & santé" },
  { id: "mode", label: "Mode & vêtements" },
  { id: "bureau", label: "Bureau & travail" },
  { id: "batiment", label: "Bâtiments & lieux" },
  { id: "espace", label: "Espace & astronomie" },
  { id: "education", label: "École & savoir" },
  { id: "communication", label: "Communication" },
  { id: "energie", label: "Énergie & écologie" },
  { id: "voyage", label: "Voyage & vacances" },
  { id: "maison", label: "Maison & intérieur" },
  { id: "beaute", label: "Beauté & soin" },
  { id: "agriculture", label: "Agriculture & ferme" },
  { id: "jeux", label: "Jeux & jouets" },
  { id: "interface", label: "Symboles d'interface" },
  { id: "art", label: "Art & culture" },
  { id: "finance", label: "Finance & commerce" },
  { id: "bebe", label: "Bébé & enfance" },
  { id: "marine", label: "Mer & vie marine" },
  { id: "metiers", label: "Métiers" },
  { id: "symboles", label: "Symboles & signes" },
  { id: "recompense", label: "Récompenses" },
  { id: "signalisation", label: "Signalisation" },
  { id: "ville", label: "Ville & mobilier urbain" },
    ...
    for (let n = 5; n <= 10; n++)
      for (const irr of [14, 22, 30])
        for (let seed = 0; seed < 20; seed++) {
          ...
          // Le décalé se peint en NUANCE SOMBRE de la couleur principale, pas
          // dans un second emplacement : celui-ci partant de la même teinte,
          // la copie tombait pile sous la forme et la variante « double »
          // était indistinguable du blob plat.
          M("blobs", `bl${i++}`, "Blob double", "blob organique double décalé relief 3d ombre", 200, 200,
            `<g transform="translate(12 12)">${fillWith(d, "__CDD__")}</g>` + fillp(d));
        }
  }
      ...
      for (const head of [18, 26, 34])
        for (const bend of [-40, -20, 0, 20, 40])
          for (const style of ["line", "solid", "dashed", "double"]) {
            ...
            const b =
              style === "solid"
                ? (() => { const yb = y0 + bend * 0.6, ym = y0 + bend * 0.3;   // la hampe suit la courbure
                    const hx = 200 - head * 2.2, hy = head * 0.5;                // …et la tête, sa taille
                    return fillp(`M 12 ${N(y0 - body)} Q 66 ${N(ym - body)} ${N(hx)} ${N(yb - body)} V ${N(yb - body - hy)} L 200 ${N(yb)} L ${N(hx)} ${N(yb + body + hy)} V ${N(yb + body)} Q 66 ${N(ym + body)} 12 ${N(y0 + body)} Z`); })()
                : style === "dashed"
      ...
      for (const amp of [6, 12, 20, 30])
        for (const sw of [3, 5, 8, 12])
          for (const seg of [3, 5, 7]) {
            ...
            const cap = (h: number) => stroke(`M 14 ${N(y - h)} V ${N(y + h)} M ${W - 14} ${N(y - h)} V ${N(y + h)}`, Math.max(2, sw * 0.8));
            const cad = (on: number) => { const q = (W - 28) / seg; return `stroke-dasharray="${N(q * on)} ${N(q * (1 - on))}"`; };
            const b =
              type === "dash" ? stroke(d, sw, cad(0.6)) + cap(amp * 0.5)
              : type === "double" ? stroke(`M 14 ${y - amp / 2} H ${W - 14}`, sw) + stroke(`M 14 ${y + amp / 2} H ${W - 14}`, sw, cad(0.7))
              : type === "beads" ? stroke(d, Math.max(2, sw / 2)) + Array.from({ length: seg + 1 }, (_, k) => `<circle cx="${N(14 + k * step)}" cy="${y}" r="${N(Math.min(sw * (0.6 + amp / 60), 11))}" fill="${C}"/>`).join("")
              : type === "line" ? stroke(d, sw) + cap(amp * 0.6)
              : stroke(d, sw);
                  ...
                  : gidx === "search"
                    ...
                    : gidx === "clock"
                      ...
                      : gidx === "mail"
                        ...
                        : `<path d="${d}" fill="${c1()}"/>`;
              const s = size * [0.88, 1, 1.1][seed];
              const inner = `<g transform="translate(${N(100 - 100 * s)} ${N(100 - 100 * s)}) scale(${N(s)})">${glyph}</g>`;
              const box =
                ...
                : cont === "badge" ? (filled ? fillp(starPts(100, 100, 20, 92, 74)) + `<circle cx="100" cy="100" r="66" fill="__CDD__"/>` : fillp(starPts(100, 100, 14, 92, 78)))
                : cont === "ring" ? (filled ? `<circle cx="100" cy="100" r="88" fill="none" stroke="${C}" stroke-width="10"/><circle cx="100" cy="100" r="70" fill="none" stroke="${C}" stroke-width="6"/>` : `<circle cx="100" cy="100" r="88" fill="none" stroke="${C}" stroke-width="14"/>`)
                : "";
              if (cont === "none" && filled === 0) continue; // doublon visuel
              // Sur un conteneur PLEIN, le symbole doit contraster : son
              // emplacement partant sinon de la teinte du conteneur, le glyphe
              // disparaissait et toutes les icônes pleines se ressemblaient.
              const solide = cont === "badge" || (filled === 1 && (cont === "circle" || cont === "square" || cont === "rounded"));
              M("icons", `ic${i++}`, glabel, `icône ${glabel} symbole pictogramme ui`, 200, 200,
                (cont === "none" ? "" : box) + inner,
                solide ? [{ label: "Couleur principale" }, { label: "Couleur du symbole", def: "#f8fafc" }] : TWO);
            }
  }
            ...
            for (const style of ["stroke", "fill"] as const) {
              ...
              for (let rr = 0; rr < rows; rr++) {
                ...
                g += style === "stroke"
                  ...
                  : `<path d="${d} V ${N(y + amp + 22)} H 10 Z" fill="${C}" opacity="${N(0.9 - rr * 0.2)}"/>` + stroke(d, sw);
              }
        ...
        for (const kind of ["grid", "check", "cross", "brick", "diag"] as const)
          for (const cols of [6, 8, 10])
            for (const rows of [4, 6]) {
              ...
              else if (kind === "check") { for (let a = 0; a < cols; a++) for (let b = 0; b < rows; b++) if ((a + b) % 2 === 0) g += `<rect x="${N(a * cell + sw / 2)}" y="${N(b * cell + sw / 2)}" width="${N(cell - sw)}" height="${N(cell - sw)}" fill="${C}"/>`; }
            ...
            for (const ring of [0, 1]) {
              ...
              if (style === "none")
                for (let k = 0; k < rays; k++) {
                  const [x, y] = polar(100, 100, core + 10 + rlen * 0.5, (k * 360) / rays);
                  g += `<circle cx="${N(x)}" cy="${N(y)}" r="${N(3 + rlen / 12)}" fill="${C}"/>`;
                }
              else
                for (let k = 0; k < rays; k++) {
                  const a = (k * 360) / rays;
        ...
        for (const hh of [60, 84, 110])
          for (const style of ["f", "o", "band", "hole"] as const)
            for (const seed of [0, 1]) {
              ...
              else { const rx = Math.min(w, 200) / 2; d = `M ${N(cx - rx)} ${N(cy)} a ${N(rx)} ${N(hh / 2)} 0 1 0 ${N(rx * 2)} 0 a ${N(rx)} ${N(hh / 2)} 0 1 0 ${N(-rx * 2)} 0 Z`; }
...
const OVERFLOW_PAD: Record<string, [number, number, number, number]> = {
  ...
  "badges.mtg170": [0, 14, 0, 4],
  "badges.mtg171": [0, 10, 0, 8],
  ...
  "badges.mtg178": [0, 14, 0, 4],
  "badges.mtg179": [0, 10, 0, 8],
  ...
  "badges.mtg186": [0, 14, 0, 4],
  "badges.mtg187": [0, 10, 0, 8],
  ...
  "badges.mtg194": [0, 34, 0, 24],
  "badges.mtg195": [0, 30, 0, 28],
  ...
  "badges.mtg202": [0, 34, 0, 24],
  "badges.mtg203": [0, 30, 0, 28],
  ...
  "badges.mtg210": [0, 34, 0, 24],
  "badges.mtg211": [0, 30, 0, 28],
  ...
  "blobs.xo23": [0, 0, 0, 2],
  "blobs.xo31": [4, 0, 0, 0],
  "circles.brush0": [0, 0, 8, 0],
  "circles.brush1": [0, 0, 3, 1],
  "circles.brush10": [0, 0, 6, 5],
  "circles.brush18": [0, 0, 4, 0],
  "circles.brush23": [0, 0, 20, 0],
  "circles.brush30": [0, 0, 30, 0],
  "circles.brush34": [0, 0, 8, 0],
  "circles.brush35": [0, 0, 0, 4],
  "circles.brush6": [0, 0, 4, 0],
  "deco.mgr102": [4, 4, 4, 4],
  "deco.mgr103": [4, 4, 4, 4],
  "deco.mgr104": [4, 4, 4, 4],
  "deco.mgr105": [4, 4, 4, 4],
  ...
  "deco.mgr196": [4, 4, 4, 4],
  "deco.mgr197": [4, 4, 4, 4],
  "deco.mgr198": [0, 0, 11, 0],
  "deco.mgr199": [0, 0, 11, 0],
  "deco.mgr20": [0, 0, 4, 0],
  "deco.mgr200": [0, 0, 11, 0],
  "deco.mgr201": [0, 0, 11, 0],
  "deco.mgr202": [0, 0, 11, 0],
  "deco.mgr203": [0, 0, 11, 0],
  "deco.mgr21": [0, 0, 4, 0],
  "deco.mgr22": [0, 0, 4, 0],
  "deco.mgr222": [5, 5, 5, 5],
  "deco.mgr223": [5, 5, 5, 5],
  ...
  "deco.mgr226": [5, 5, 5, 5],
  "deco.mgr227": [5, 5, 5, 5],
  "deco.mgr228": [0, 0, 12, 0],
  "deco.mgr229": [0, 0, 12, 0],
  "deco.mgr23": [0, 0, 4, 0],
  "deco.mgr230": [0, 0, 12, 0],
  "deco.mgr231": [0, 0, 12, 0],
  "deco.mgr232": [0, 0, 12, 0],
  "deco.mgr233": [0, 0, 12, 0],
  "deco.mgr240": [1, 1, 1, 1],
  "deco.mgr241": [1, 1, 1, 1],
  "deco.mgr242": [1, 1, 1, 1],
  "deco.mgr243": [1, 1, 1, 1],
  ...
  "deco.mgr256": [6, 6, 6, 6],
  "deco.mgr257": [6, 6, 6, 6],
  "deco.mgr258": [1, 1, 13, 0],
  "deco.mgr259": [1, 1, 13, 0],
  "deco.mgr260": [1, 1, 13, 0],
  "deco.mgr261": [1, 1, 13, 0],
  "deco.mgr262": [1, 1, 13, 0],
  "deco.mgr263": [1, 1, 13, 0],
  "deco.mgr282": [4, 4, 4, 4],
  "deco.mgr283": [4, 4, 4, 4],
  "deco.mgr284": [4, 4, 4, 4],
  "deco.mgr285": [4, 4, 4, 4],
  "deco.mgr286": [4, 4, 4, 4],
  "deco.mgr287": [4, 4, 4, 4],
  "deco.mgr288": [0, 0, 16, 0],
  "deco.mgr289": [0, 0, 16, 0],
  "deco.mgr290": [0, 0, 16, 0],
  "deco.mgr291": [0, 0, 16, 0],
  "deco.mgr292": [0, 0, 16, 0],
  "deco.mgr293": [0, 0, 16, 0],
  "deco.mgr312": [5, 5, 5, 5],
  "deco.mgr313": [5, 5, 5, 5],
  "deco.mgr314": [5, 5, 5, 5],
  "deco.mgr315": [5, 5, 5, 5],
  "deco.mgr316": [5, 5, 5, 5],
  "deco.mgr317": [5, 5, 5, 5],
  "deco.mgr318": [0, 0, 17, 0],
  "deco.mgr319": [0, 0, 17, 0],
  "deco.mgr320": [0, 0, 17, 0],
  "deco.mgr321": [0, 0, 17, 0],
  "deco.mgr322": [0, 0, 17, 0],
  "deco.mgr323": [0, 0, 17, 0],
  "deco.mgr330": [1, 1, 1, 1],
  "deco.mgr331": [1, 1, 1, 1],
  "deco.mgr332": [1, 1, 1, 1],
  "deco.mgr333": [1, 1, 1, 1],
  ...
  "deco.mgr346": [6, 6, 6, 6],
  "deco.mgr347": [6, 6, 6, 6],
  "deco.mgr348": [1, 1, 18, 0],
  "deco.mgr349": [1, 1, 18, 0],
  "deco.mgr350": [1, 1, 18, 0],
  "deco.mgr351": [1, 1, 18, 0],
  "deco.mgr352": [1, 1, 18, 0],
  "deco.mgr353": [1, 1, 18, 0],
  "deco.mgr42": [5, 5, 5, 5],
  "deco.mgr43": [5, 5, 5, 5],
  "deco.mgr44": [5, 5, 5, 5],
  "deco.mgr45": [5, 5, 5, 5],
  ...
  "deco.mpat406": [2, 0, 0, 0],
  "deco.mpat414": [2, 2, 0, 0],
  "deco.mpat418": [9, 8, 0, 0],
  "deco.mpat42": [3, 8, 0, 0],
  "deco.mpat422": [4, 5, 0, 0],
  "deco.mpat426": [0, 11, 0, 0],
  "deco.mpat428": [6, 6, 0, 0],
  "deco.mpat429": [6, 6, 0, 0],
  "deco.mpat430": [2, 4, 0, 0],
  "deco.mpat431": [6, 6, 0, 0],
  "deco.mpat44": [2, 2, 0, 0],
  "deco.mpat45": [2, 2, 0, 0],
  ...
  "deco.mpat70": [0, 2, 0, 0],
  "deco.mpat82": [6, 10, 0, 0],
  "deco.mpat86": [0, 1, 0, 0],
  "deco.mpat90": [7, 11, 0, 0],
  "deco.mpat92": [6, 6, 0, 0],
  "deco.mpat93": [6, 6, 0, 0],
  "deco.mpat94": [1, 19, 0, 0],
  "deco.mpat95": [6, 6, 0, 0],
  "deco.mwv10": [0, 0, 58, 0],
  "deco.mwv100": [0, 0, 58, 0],
  "deco.mwv101": [0, 0, 58, 0],
  "deco.mwv104": [0, 0, 36, 0],
  "deco.mwv105": [0, 0, 36, 0],
  "deco.mwv106": [0, 0, 60, 0],
  "deco.mwv107": [0, 0, 60, 0],
  "deco.mwv11": [0, 0, 58, 0],
  "deco.mwv110": [0, 0, 32, 0],
  "deco.mwv111": [0, 0, 32, 0],
  "deco.mwv112": [0, 0, 56, 0],
  "deco.mwv113": [0, 0, 56, 0],
  "deco.mwv116": [0, 0, 34, 0],
  "deco.mwv117": [0, 0, 34, 0],
  "deco.mwv118": [0, 0, 58, 0],
  "deco.mwv119": [0, 0, 58, 0],
  "deco.mwv122": [0, 0, 36, 0],
  "deco.mwv123": [0, 0, 36, 0],
  "deco.mwv124": [0, 0, 60, 0],
  "deco.mwv125": [0, 0, 60, 0],
  "deco.mwv128": [0, 0, 32, 0],
  "deco.mwv129": [0, 0, 32, 0],
  "deco.mwv130": [0, 0, 56, 0],
  "deco.mwv131": [0, 0, 56, 0],
  "deco.mwv134": [0, 0, 34, 0],
  "deco.mwv135": [0, 0, 34, 0],
  "deco.mwv136": [0, 0, 58, 0],
  "deco.mwv137": [0, 0, 58, 0],
  "deco.mwv14": [0, 0, 36, 0],
  "deco.mwv140": [0, 0, 36, 0],
  "deco.mwv141": [0, 0, 36, 0],
  "deco.mwv142": [0, 0, 60, 0],
  "deco.mwv143": [0, 0, 60, 0],
  "deco.mwv146": [0, 0, 32, 0],
  "deco.mwv147": [0, 0, 32, 0],
  "deco.mwv148": [0, 0, 56, 0],
  "deco.mwv149": [0, 0, 56, 0],
  "deco.mwv15": [0, 0, 36, 0],
  "deco.mwv152": [0, 0, 34, 0],
  "deco.mwv153": [0, 0, 34, 0],
  "deco.mwv154": [0, 0, 58, 0],
  "deco.mwv155": [0, 0, 58, 0],
  "deco.mwv158": [0, 0, 36, 0],
  "deco.mwv159": [0, 0, 36, 0],
  "deco.mwv16": [0, 0, 60, 0],
  "deco.mwv160": [0, 0, 60, 0],
  "deco.mwv161": [0, 0, 60, 0],
  "deco.mwv164": [0, 0, 32, 0],
  "deco.mwv165": [0, 0, 32, 0],
  "deco.mwv166": [0, 0, 56, 0],
  "deco.mwv167": [0, 0, 56, 0],
  "deco.mwv17": [0, 0, 60, 0],
  "deco.mwv170": [0, 0, 34, 0],
  "deco.mwv171": [0, 0, 34, 0],
  "deco.mwv172": [0, 0, 58, 0],
  "deco.mwv173": [0, 0, 58, 0],
  "deco.mwv176": [0, 0, 36, 0],
  "deco.mwv177": [0, 0, 36, 0],
  "deco.mwv178": [0, 0, 60, 0],
  "deco.mwv179": [0, 0, 60, 0],
  "deco.mwv182": [0, 0, 32, 0],
  "deco.mwv183": [0, 0, 32, 0],
  "deco.mwv184": [0, 0, 56, 0],
  "deco.mwv185": [0, 0, 56, 0],
  "deco.mwv188": [0, 0, 34, 0],
  "deco.mwv189": [0, 0, 34, 0],
  "deco.mwv190": [0, 0, 58, 0],
  "deco.mwv191": [0, 0, 58, 0],
  "deco.mwv194": [0, 0, 36, 0],
  "deco.mwv195": [0, 0, 36, 0],
  "deco.mwv196": [0, 0, 60, 0],
  "deco.mwv197": [0, 0, 60, 0],
  "deco.mwv2": [0, 0, 32, 0],
  "deco.mwv20": [0, 0, 32, 0],
  "deco.mwv200": [0, 0, 32, 0],
  "deco.mwv201": [0, 0, 32, 0],
  "deco.mwv202": [0, 0, 56, 0],
  "deco.mwv203": [0, 0, 56, 0],
  "deco.mwv206": [0, 0, 34, 0],
  "deco.mwv207": [0, 0, 34, 0],
  "deco.mwv208": [0, 0, 58, 0],
  "deco.mwv209": [0, 0, 58, 0],
  "deco.mwv21": [0, 0, 32, 0],
  "deco.mwv212": [0, 0, 36, 0],
  "deco.mwv213": [0, 0, 36, 0],
  "deco.mwv214": [0, 0, 60, 0],
  "deco.mwv215": [0, 0, 60, 0],
  "deco.mwv218": [0, 0, 32, 0],
  "deco.mwv219": [0, 0, 32, 0],
  "deco.mwv22": [0, 0, 56, 0],
  "deco.mwv220": [0, 0, 56, 0],
  "deco.mwv221": [0, 0, 56, 0],
  "deco.mwv224": [0, 0, 34, 0],
  "deco.mwv225": [0, 0, 34, 0],
  "deco.mwv226": [0, 0, 58, 0],
  "deco.mwv227": [0, 0, 58, 0],
  "deco.mwv23": [0, 0, 56, 0],
  "deco.mwv230": [0, 0, 36, 0],
  "deco.mwv231": [0, 0, 36, 0],
  "deco.mwv232": [0, 0, 60, 0],
  "deco.mwv233": [0, 0, 60, 0],
  "deco.mwv236": [0, 0, 32, 0],
  "deco.mwv237": [0, 0, 32, 0],
  "deco.mwv238": [0, 0, 56, 0],
  "deco.mwv239": [0, 0, 56, 0],
  "deco.mwv242": [0, 0, 34, 0],
  "deco.mwv243": [0, 0, 34, 0],
  "deco.mwv244": [0, 0, 58, 0],
  "deco.mwv245": [0, 0, 58, 0],
  "deco.mwv248": [0, 0, 36, 0],
  "deco.mwv249": [0, 0, 36, 0],
  "deco.mwv250": [0, 0, 60, 0],
  "deco.mwv251": [0, 0, 60, 0],
  "deco.mwv254": [0, 0, 32, 0],
  "deco.mwv255": [0, 0, 32, 0],
  "deco.mwv256": [0, 0, 56, 0],
  "deco.mwv257": [0, 0, 56, 0],
  "deco.mwv26": [0, 0, 34, 0],
  "deco.mwv260": [0, 0, 34, 0],
  "deco.mwv261": [0, 0, 34, 0],
  "deco.mwv262": [0, 0, 58, 0],
  "deco.mwv263": [0, 0, 58, 0],
  "deco.mwv266": [0, 0, 36, 0],
  "deco.mwv267": [0, 0, 36, 0],
  "deco.mwv268": [0, 0, 60, 0],
  "deco.mwv269": [0, 0, 60, 0],
  "deco.mwv27": [0, 0, 34, 0],
  "deco.mwv272": [0, 0, 32, 0],
  "deco.mwv273": [0, 0, 32, 0],
  "deco.mwv274": [0, 0, 56, 0],
  "deco.mwv275": [0, 0, 56, 0],
  "deco.mwv278": [0, 0, 34, 0],
  "deco.mwv279": [0, 0, 34, 0],
  "deco.mwv28": [0, 0, 58, 0],
  "deco.mwv280": [0, 0, 58, 0],
  "deco.mwv281": [0, 0, 58, 0],
  "deco.mwv284": [0, 0, 36, 0],
  "deco.mwv285": [0, 0, 36, 0],
  "deco.mwv286": [0, 0, 60, 0],
  "deco.mwv287": [0, 0, 60, 0],
  "deco.mwv29": [0, 0, 58, 0],
  "deco.mwv3": [0, 0, 32, 0],
  "deco.mwv32": [0, 0, 36, 0],
  "deco.mwv33": [0, 0, 36, 0],
  ...
  "geo.mv3145": [0, 0, 0, 2],
  "geo.mv3149": [0, 0, 0, 2],
  "geo.shieldgo": [0, 0, 0, 5],
  "icons.stargroup1": [0, 0, 8, 0],
  "nature.hddroplet": [0, 0, 0, 10],
  "nature.mdr152": [0, 1, 0, 0],
  "nature.mdr153": [0, 1, 0, 0],
  "nature.mdr154": [0, 1, 0, 0],
  "nature.mdr155": [0, 1, 0, 0],
  "nature.mdr156": [0, 1, 0, 0],
  "nature.mdr157": [0, 1, 0, 0],
  "nature.mdr158": [0, 1, 0, 0],
  "nature.mdr159": [0, 1, 0, 0],
  "nature.mdr176": [0, 1, 0, 0],
  "nature.mdr177": [0, 1, 0, 0],
  "nature.mdr178": [0, 1, 0, 0],
  "nature.mdr179": [0, 1, 0, 0],
  "nature.mdr180": [0, 1, 0, 0],
  "nature.mdr181": [0, 1, 0, 0],
  "nature.mdr182": [0, 1, 0, 0],
  "nature.mdr183": [0, 1, 0, 0],
  "nature.mdr200": [0, 1, 0, 0],
  "nature.mdr201": [0, 1, 0, 0],
  "nature.mdr202": [0, 1, 0, 0],
  "nature.mdr203": [0, 1, 0, 0],
  "nature.mdr204": [0, 1, 0, 0],
  "nature.mdr205": [0, 1, 0, 0],
  "nature.mdr206": [0, 1, 0, 0],
  "nature.mdr207": [0, 1, 0, 0],
  "nature.mount2": [0, 0, 12, 0],
  "nature.palm1": [0, 3, 0, 0],
  "nature.pine1": [0, 0, 38, 0],
  "nature.waterdrop1": [0, 0, 0, 8],
  "objects.balloon2": [0, 0, 0, 6],
  "objects.clock1": [0, 0, 0, 1],
  ...
  "party.balloon0": [0, 0, 0, 4],
  "party.balloon3": [0, 0, 0, 12],
  "party.buntingx": [0, 0, 2, 0],
  "party.confetti1": [0, 0, 0, 3],
  "party.confetti15": [0, 2, 0, 0],
  "party.confetti5": [0, 0, 5, 0],
  "party.mcf122": [0, 0, 1, 0],
  "party.mcf126": [0, 0, 5, 0],
  "party.mcf131": [0, 0, 7, 0],
  "party.mcf179": [0, 0, 1, 0],
  "party.mcf2": [0, 0, 4, 0],
  "party.mcf217": [0, 0, 7, 0],
  "party.mcf224": [0, 0, 5, 0],
  "party.mcf241": [0, 0, 8, 0],
  "party.mcf290": [0, 0, 0, 2],
  "party.mcf291": [0, 0, 0, 1],
  "party.mcf340": [0, 0, 7, 0],
  "party.mcf342": [0, 0, 5, 0],
  "party.mcf343": [0, 0, 1, 0],
  "party.mcf344": [0, 0, 6, 0],
  "party.mcf345": [0, 0, 8, 0],
  "party.mcf346": [0, 0, 6, 0],
  "party.mcf362": [0, 0, 2, 0],
  "party.mcf364": [0, 0, 1, 0],
  "party.mcf365": [0, 0, 2, 0],
  "party.mcf367": [0, 0, 4, 0],
  "party.mcf368": [0, 0, 4, 0],
  "party.mcf371": [0, 0, 3, 0],
  "party.mcf410": [0, 0, 1, 0],
  "party.mcf456": [0, 0, 2, 0],
  "party.mcf459": [0, 0, 10, 0],
  "party.mcf460": [0, 0, 5, 0],
  "party.mcf464": [0, 0, 2, 0],
  "party.mcf52": [0, 0, 0, 4],
  "party.mcf7": [0, 0, 4, 0],
  "party.mcf9": [0, 0, 4, 0],
  "party.mcf99": [0, 0, 6, 0],
  "party.streamer0": [0, 17, 14, 0],
  "party.streamer1": [0, 25, 12, 0],
  "party.streamer2": [0, 24, 11, 0],
  "party.streamer3": [0, 23, 8, 0],
  "party.streamer4": [0, 11, 4, 0],
  "party.streamer5": [0, 17, 8, 0],
  "party.streamer6": [0, 3, 15, 0],
  "party.streamer7": [0, 22, 5, 0],
  "sport.jumprope0": [0, 0, 0, 5],
  "stars.mex0": [0, 0, 0, 1],
  "stars.mex1": [0, 0, 0, 1],
  "stars.mex10": [6, 0, 3, 0],
  "stars.mex100": [0, 1, 0, 0],
  "stars.mex101": [0, 3, 0, 0],
  "stars.mex105": [1, 5, 0, 0],
  "stars.mex106": [0, 3, 0, 0],
  "stars.mex107": [1, 0, 0, 0],
  "stars.mex108": [0, 6, 0, 0],
  "stars.mex109": [1, 0, 0, 0],
  "stars.mex110": [0, 4, 0, 0],
  "stars.mex115": [0, 0, 0, 2],
  "stars.mex116": [0, 1, 0, 0],
  "stars.mex118": [0, 2, 0, 0],
  "stars.mex119": [0, 1, 0, 0],
  "stars.mex12": [0, 0, 0, 1],
  "stars.mex128": [0, 1, 0, 0],
  "stars.mex129": [0, 0, 1, 6],
  "stars.mex13": [0, 0, 5, 1],
  "stars.mex131": [0, 1, 0, 0],
  "stars.mex132": [1, 0, 0, 0],
  "stars.mex133": [0, 1, 0, 0],
  "stars.mex134": [1, 2, 0, 3],
  "stars.mex145": [0, 0, 0, 2],
  "stars.mex146": [0, 0, 1, 0],
  "stars.mex148": [0, 0, 3, 0],
  "stars.mex150": [0, 0, 3, 0],
  "stars.mex151": [0, 0, 0, 1],
  "stars.mex152": [3, 0, 0, 0],
  "stars.mex154": [4, 0, 0, 0],
  "stars.mex155": [4, 0, 5, 1],
  "stars.mex156": [0, 0, 0, 1],
  "stars.mex157": [0, 0, 4, 4],
  "stars.mex158": [5, 2, 0, 0],
  "stars.mex159": [0, 5, 0, 0],
  ...
  "stars.mex176": [0, 0, 4, 0],
  "stars.mex179": [0, 0, 0, 2],
  "stars.mex180": [0, 0, 5, 0],
  "stars.mex181": [0, 5, 0, 0],
  "stars.mex183": [0, 2, 0, 6],
  ...
  "stars.mex2": [0, 0, 2, 0],
  "stars.mex200": [0, 3, 0, 1],
  "stars.mex201": [4, 0, 0, 0],
  "stars.mex202": [4, 0, 0, 0],
  "stars.mex203": [4, 0, 0, 0],
  "stars.mex205": [0, 0, 0, 3],
  "stars.mex206": [6, 0, 0, 6],
  "stars.mex207": [0, 0, 6, 0],
  "stars.mex21": [0, 0, 0, 3],
  ...
  "stars.mex224": [0, 0, 0, 2],
  "stars.mex225": [0, 0, 6, 0],
  "stars.mex226": [4, 0, 0, 3],
  "stars.mex227": [6, 0, 0, 0],
  "stars.mex228": [0, 0, 4, 3],
  "stars.mex230": [2, 0, 0, 3],
  "stars.mex231": [0, 0, 0, 6],
  ...
  "stars.mex249": [4, 0, 0, 0],
  "stars.mex250": [2, 0, 3, 0],
  "stars.mex251": [3, 3, 0, 0],
  "stars.mex253": [0, 2, 0, 0],
  "stars.mex255": [5, 4, 0, 0],
  ...
  "stars.mex273": [0, 1, 3, 0],
  "stars.mex274": [0, 1, 0, 0],
  "stars.mex275": [1, 5, 0, 0],
  "stars.mex276": [3, 0, 2, 2],
  "stars.mex277": [1, 0, 0, 0],
  "stars.mex278": [0, 4, 3, 0],
  "stars.mex279": [5, 6, 1, 0],
  "stars.mex285": [0, 0, 1, 0],
  "stars.mex286": [0, 0, 1, 0],
  "stars.mex288": [0, 0, 2, 0],
  "stars.mex29": [0, 2, 0, 0],
  "stars.mex294": [0, 0, 1, 0],
  "stars.mex297": [6, 0, 0, 0],
  "stars.mex298": [4, 0, 0, 0],
  "stars.mex299": [2, 0, 0, 0],
  "stars.mex3": [0, 3, 2, 0],
  "stars.mex301": [0, 2, 0, 0],
  "stars.mex302": [0, 0, 0, 2],
  "stars.mex303": [0, 3, 1, 3],
  ...
  "stars.mex320": [0, 0, 0, 3],
  "stars.mex321": [1, 0, 0, 2],
  "stars.mex322": [2, 0, 0, 0],
  "stars.mex323": [0, 0, 5, 1],
  "stars.mex325": [0, 0, 0, 1],
  "stars.mex326": [0, 0, 1, 4],
  "stars.mex327": [0, 0, 4, 0],
  ...
  "stars.mex345": [0, 1, 0, 3],
  "stars.mex346": [2, 5, 6, 2],
  "stars.mex347": [0, 0, 0, 1],
  "stars.mex349": [0, 0, 4, 0],
  "stars.mex35": [0, 0, 0, 5],
  "stars.mex350": [0, 0, 2, 6],
  ...
  "stars.mex96": [0, 3, 0, 0],
  "stars.mex99": [0, 0, 0, 2],
  "strokes.thick5": [1, 0, 1, 0],
  "tech.circuit2": [0, 0, 0, 1],
  "tech.circuit3": [0, 2, 0, 0],
  "tech.circuit4": [0, 4, 0, 0],
  "tech.circuit6": [0, 2, 0, 1],
  "tech.circuit9": [0, 0, 0, 1],
  "weather.msn102": [6, 6, 6, 6],
  "weather.msn103": [6, 6, 6, 6],
  "weather.msn104": [2, 2, 2, 2],
  "weather.msn105": [2, 2, 2, 2],
  "weather.msn156": [6, 1, 6, 1],
  "weather.msn157": [6, 1, 6, 1],
  "weather.msn158": [2, 0, 2, 0],
  "weather.msn159": [2, 0, 2, 0],
  "weather.msn210": [6, 6, 6, 6],
  "weather.msn211": [6, 6, 6, 6],
  "weather.msn212": [2, 2, 2, 2],
  "weather.msn213": [2, 2, 2, 2],
  "weather.msn264": [6, 6, 6, 6],
  "weather.msn265": [6, 6, 6, 6],
  "weather.msn266": [2, 2, 2, 2],
  "weather.msn267": [2, 2, 2, 2],
  "weather.msn318": [6, 6, 6, 6],
  "weather.msn319": [6, 6, 6, 6],
  "weather.msn320": [2, 2, 2, 2],
  "weather.msn321": [2, 2, 2, 2],
  "weather.msn48": [6, 0, 6, 0],
  "weather.msn49": [6, 0, 6, 0],
  "weather.msn50": [2, 0, 2, 0],
  "weather.msn51": [2, 0, 2, 0],
  "weather.tornado1": [0, 0, 0, 2],
};
for (const e of items) {
